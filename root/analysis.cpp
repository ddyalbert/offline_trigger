...
void analysis()
{
    ...
    TH1D *hist_E = new TH1D("hist_E", "Hist;E(MeV)", 250, 0, 3);
    ...
    for (size_t i = 0; i < n; i++)
    {
        ...
        graph_amp_RT->AddPoint(amp, RT);
        graph_amp_DT->AddPoint(amp, DT);
        graph_amp_chi2->AddPoint(amp, chi2_fil);
        // graph_amp_raw_chi2->AddPoint(amp, chi2_raw);

        n_cut++;

        Double_t E = 1.30 * amp - 0.03;
        if(E < 0) continue;
    ...
    // TCanvas *canvas_raw_fil = new TCanvas("canvas_raw_fil", "raw - fil");
    ...
    // TF1 *guss1 = new TF1("guss1", "[0] * TMath::Gaus(x, [1], [2]) + [3] + [4] * x", 1.26, 1.28);
    // guss1->SetParNames("Amplitude", "Mean", "Sigma");
    // guss1->SetParameters(1, 1.27, 0.01, 10, 0);
    // hist_E->Fit(guss1,"R+");

    // TF1 *guss2 = new TF1("guss2", "[0] * TMath::Gaus(x, [1], [2]) + [3] * exp( - [4] * x )", 0.4, 0.6);
    // guss2->SetParNames("Amplitude", "Mean", "Sigma");
    // guss2->SetParameters(10, 0.51, 0.01, 100, 0);
    // hist_E->Fit(guss2,"R+");
